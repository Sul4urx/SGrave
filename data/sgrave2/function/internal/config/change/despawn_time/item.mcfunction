...
execute unless predicate {\
  ...
  "translate": "sgrave2.change_config_message.despawn_time.item.fail",\
  "fallback": "§cThe value must be a non-negative integer."\
}

## Otherwise success
title @s actionbar {\
  "translate": "sgrave2.change_config_message.despawn_time.item.success",\
  "fallback": "Successfully changed the Item Despawn Time config from %s seconds to %s seconds.",\
  "with": [\
    {"storage": "sgrave2:common", "nbt": "configs.despawn_time.item.value"},\
    {"storage": "sgrave2:common", "nbt": "temp.config.value"}\
  ]\
}

## If success, change value
execute store result score <despawn_time.item> sgrave2.config run data get storage sgrave2:common temp.config.value
