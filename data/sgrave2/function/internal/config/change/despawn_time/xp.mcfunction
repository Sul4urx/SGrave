...
title @s actionbar {\
  ...
  "with": [\
    {"storage": "sgrave2:common", "nbt": "configs.despawn_time.xp.value"},\
    {"storage": "sgrave2:common", "nbt": "temp.config.value"}\
  ]\
}

## If success, change value
execute store result score (config:dropped_contents/xp_despawn_time) sgrave2.config run data get storage sgrave2:common temp.config.value
