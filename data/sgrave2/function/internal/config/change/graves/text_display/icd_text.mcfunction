...
execute if data entity @n[tag=sgrave2.temp.config_checker.text_display] text{text: "a", color: "green"} run title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.icd_text.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.icd_text.success",\
  "fallback": "Successfully changed ICD Text."\
}

## If success, change value
data modify storage sgrave2:common configs.value.graves.text_display.icd_text set from storage sgrave2:common temp.config.value
