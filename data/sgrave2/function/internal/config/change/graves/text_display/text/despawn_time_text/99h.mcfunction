...
execute if data entity @n[tag=sgrave2.temp.config_checker.text_display] {text: '"a"'} run title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.text.despawn_time_text.99+_hours.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.text.despawn_time_text.99+_hours.success",\
  "fallback": "Successfully changed Despawn Time Text / 99+ Hours."\
}

## If success, change value
data modify storage sgrave2:common configs.value.graves.text_display.text.despawn_time_text.99+_hours set from storage sgrave2:common temp.config.value
