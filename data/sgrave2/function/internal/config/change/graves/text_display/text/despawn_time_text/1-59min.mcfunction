...
execute if data entity @n[tag=sgrave2.temp.config_checker.text_display] {text: '"a"'} run title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.text.despawn_time_text.1-59_minutes.fail",\
...
## Otherwise success
title @s actionbar {\
  "translate": "sgrave2.change_config_message.text_display.text.despawn_time_text.1-59_minutes.success",\
  "fallback": "Successfully changed Despawn Time Text / 1-59 Minutes."\
}

## If success, change value
data modify storage sgrave2:common configs.value.graves.text_display.text.despawn_time_text.1-59_minutes set from storage sgrave2:common temp.config.value
