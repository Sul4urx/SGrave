...
execute unless predicate {\
  ...
  "translate": "sgrave2.change_config_message.despawn_time.grave.fail",\
...
title @s actionbar {\
  ...
  "with": [\
    {"storage": "sgrave2:common", "nbt": "configs.value.graves.despawn_time"},\
    {"storage": "sgrave2:common", "nbt": "temp.config.value"}\
  ]\
}

## If success, change value
execute store result score (graves/despawn_time) sgrave2.config run data get storage sgrave2:common temp.config.value
