...
execute unless predicate {\
  ...
  "translate": "sgrave2.change_config_message.graves.xp_distribution.grave_generation_fail.remove.fail",\
  "fallback": "§cThe value must be an integer between 0 and 25 (inclusive)."\
}

## Otherwise success
title @s actionbar {\
  "translate": "sgrave2.change_config_message.graves.xp_distribution.grave_generation_fail.remove.success",\
  "fallback": "Successfully changed the 'remove' operation weight from %s to %s.",\
  "with": [\
    {"storage": "sgrave2:common", "nbt": "configs.graves.xp_distribution.grave_generation_fail.remove.value"},\
    {"storage": "sgrave2:common", "nbt": "temp.config.value"}\
  ]\
}

## If success, change value
execute store result score (graves/xp_distribution/grave_generation_fail/remove) sgrave2.config run data get storage sgrave2:common temp.config.value
