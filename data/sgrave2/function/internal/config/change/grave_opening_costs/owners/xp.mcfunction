...
execute unless predicate {\
  ...
  "translate": "sgrave2.change_config_message.grave_opening_costs.owners.xp.fail",\
  "fallback": "§Value must be a non-negative integer."\
}

## Otherwise change values
execute store result score (config:costs/grave_opening_costs/owners/xp) sgrave2.config run data get storage sgrave2:common temp.config.levels
