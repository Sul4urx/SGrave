#<< config/change/players/blacklist_items/change_list
...
execute if data storage sgrave2:common temp.value.operation unless data storage sgrave2:common temp.value{operation:'keep'} unless data storage sgrave2:common temp.value{operation:'delete'} run return run scoreboard players set .predicate_is_valid sgrave2.temp_var 0
