...
scoreboard players set .slot sgrave2.temp_var -32768
execute if data entity @s item.components.minecraft:custom_data.sgrave2:common.items[-1].Slot store result score .slot sgrave2.temp_var run data get entity @s item.components.minecraft:custom_data.sgrave2:common.items[-1].Slot 1
...
execute unless score .slot sgrave2.temp_var matches -32768 store result entity @s item.components.minecraft:custom_data.sgrave2:common.items[-1].Slot int 1 run scoreboard players get .slot sgrave2.temp_var

## Put item in correct slot of player's inventory
execute unless score .slot sgrave2.temp_var matches -32768 run function sgrave2:internal/grave/open/give_items/put_item_in_correct_slot/main with entity @s item.components.minecraft:custom_data.sgrave2:common.items[-1]
...
execute if score .slot sgrave2.temp_var matches -32768 run data modify storage sgrave2:common temp.items_to_summon_as_entity append from entity @s item.components.minecraft:custom_data.sgrave2:common.items[-1]
