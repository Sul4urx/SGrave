...
## Items
execute if data storage sgrave2:common configs.value.costs.grave_generation_costs.with_item.items[] run data modify storage sgrave2:common temp.config_copy.items set from storage sgrave2:common configs.value.costs.grave_generation_costs.with_item.items
data modify storage sgrave2:common temp.args.value set from storage sgrave2:common temp.config_copy.items[-1]


execute store result score .loop_count sgrave2.temp_var if data storage sgrave2:common configs.value.costs.grave_generation_costs.with_item.items[]
...
execute store result score .xp_cost_levels sgrave2.temp_var run data get storage sgrave2:common configs.value.costs.grave_generation_costs.with_item.xp.levels
